<template>
    <v-container class="mb-10">
        ...
        <div class="d-flex justify-space-between">
            <h1>{{ title }}</h1>
            <div>
                <v-btn v-if="DataCart.length !== 0 || send == true" outlined class="mb-5 pt-2 pb-2 clearBtn" @click="toCatalog">Вернуться к покупкам</v-btn>
            </div>
        </div>
        <v-divider class="mb-8" />
        <div v-if="DataCart.length == 0 && send == false" style="padding: 120px 0;" class="text-center">
